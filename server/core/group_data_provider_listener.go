...
func (g *GroupDataProviderListenerImpl) OnGroupAdded(fabricIndex lib.FabricIndex, newGroup *credentials.GroupInfo) {
	...
	if err := g.mServer.GetTransportManager().MulticastGroupJoinLeave(message.Multicast(fabric.GetFabricId(), newGroup.Id), true); err != nil {
...
func (g *GroupDataProviderListenerImpl) OnGroupRemoved(fabricIndex lib.FabricIndex, newGroup *credentials.GroupInfo) {
	...
	_ = g.mServer.GetTransportManager().MulticastGroupJoinLeave(message.Multicast(fabric.GetFabricId(), newGroup.Id), false)
}

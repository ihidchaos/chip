...
import (
	...
	"github.com/galenliu/chip/app/server/dnssd"
...
const allSupported CommissioningWindowAdvertisement = 0
const dnssdOnly CommissioningWindowAdvertisement = 1
...
type Server struct {
	...
	mCASESessionManager *core.CASESessionManager
	mCASEClientPool     *core.CASEClientPool
	...
	mDeviceStorage                 store.PersistentStorageDelegate
	...
	mTestEventTriggerDelegate core.TestEventTriggerDelegate
	...
	mDevicePool  core.OperationalDeviceProxyPool
	mAclStorage  core.AclStorage
...
func NewServer() *Server {
	return &Server{
		mInterfaceId:              net.Interface{},
		mTestEventTriggerDelegate: core.TestEventTriggerDelegate{},
		mCASEClientPool:           core.NewCASEClientPool(config.DeviceMaxActiveCASEClients),
		mCASEServer:               sc.NewCASEServer(),
		mMessageCounterManager:    sc.NewMessageCounterManager(),
		mCASESessionManager:       core.NewCASESessionManager(),
		mDevicePool:               core.OperationalDeviceProxyPool{},
	}
}
...
func (s *Server) Init(initParams *InitParams) error {
	...
	{
		fabricDelegate := core.NewServerFabricDelegateImpl()
	...
	caseSessionManagerConfig := &core.CASESessionManagerConfig{
		SessionInitParams: core.DeviceProxyInitParams{
	...
	err = s.mCASESessionManager.Init(core.SystemLayer(), caseSessionManagerConfig)
	if err != nil {
		return err
	}
